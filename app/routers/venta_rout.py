...
from app.schema.venta_schem import VentaRegistro, VentaResponse, VentaDetalleResponse, VentaPagoResponse
from app.models.venta_model import VentaPago
from app.dependencies import require_permission

router = APIRouter(
    prefix="/ventas",
    tags=["ventas"],
    dependencies=[Depends(require_permission("modulo:ventas"))]
)
...
def registrar_venta(request: Request, data: VentaRegistro, db: Session = Depends(get_db)):
    ...
    return crear_venta(db, data.cliente_id, detalles, data.metodo_pago,
                       ...
                       data.id_transaccion_api, data.proveedor_pago)
...
@router.get("/{venta_id}/pagos", response_model=List[VentaPagoResponse])
def listar_pagos_venta(venta_id: int, db: Session = Depends(get_db)):
    return db.query(VentaPago).filter(VentaPago.venta_id == venta_id).all()